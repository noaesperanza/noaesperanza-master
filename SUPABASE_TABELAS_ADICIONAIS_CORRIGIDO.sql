-- MEDCANLAB 3.0 - TABELAS ADICIONAIS (CORRIGIDO)
...
-- 8. VERIFICAR E CRIAR/ATUALIZAR TABELA DE AVALIACOES CLINICAS
-- Primeiro, verificar se a tabela existe e adicionar colunas faltantes
DO $$ 
BEGIN
  -- Se a tabela não existe, criar
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'clinical_assessments') THEN
  ...
  ELSE
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
      ...
      ALTER TABLE clinical_assessments ADD COLUMN doctor_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'clinical_assessments' AND column_name = 'completed_at') THEN
      ALTER TABLE clinical_assessments ADD COLUMN completed_at TIMESTAMP WITH TIME ZONE;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'clinical_assessments' AND column_name = 'updated_at') THEN
...
-- 11. POLITICAS RLS BASICAS (DROP IF EXISTS primeiro para evitar erros)
DROP POLICY IF EXISTS "Users can view their own clinical reports" ON clinical_reports;
DROP POLICY IF EXISTS "Doctors can create clinical reports" ON clinical_reports;
DROP POLICY IF EXISTS "Professionals can view patient KPIs" ON clinical_kpis;
DROP POLICY IF EXISTS "Professionals can create KPIs" ON clinical_kpis;
DROP POLICY IF EXISTS "Users can view patient profiles" ON patient_profiles;
DROP POLICY IF EXISTS "Anyone can view published documents" ON documents;
...
DROP POLICY IF EXISTS "Users can view their clinical assessments" ON clinical_assessments;
